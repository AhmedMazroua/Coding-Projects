...
def index():
    ...
    # retrieve portfolio of stock/shares
    stocks = db.execute(
    ...
    # generate value for total
    total_value = cash

    # gather price of stocks
    for stock in stocks:
    ...
    return render_template(
        "index.html", cash=usd(cash), stocks=stocks, total_value=usd(total_value)
    )
...
def buy():
    ...
    if request.method == "POST":
        ...
        quote = lookup(symbol)
        if quote is None:
            return apology("Symbol Unidentified")
        ...
        flash(f"{shares} shares of {symbol} purchased for {usd(total_cost)}!")
        return redirect("/")
...
def history():
    ...
    # Queue sql for users transactions
...
def register():
    """Register user"""
    # clears any session id
    session.clear()

    # handles post method for register
    if request.method == "POST":
        # ensures a passord, username, confirmation passord is given, else returns apology
        if not request.form.get("username"):
        ...
        # generates newly created user
        rows = db.execute(
            "SELECT * FROM users WHERE username = ?", request.form.get("username")
        )

        # identifies whos session cookie
        session["user_id"] = rows[0]["id"]
        ...
        for stock in stocks:
            if stock["symbol"] == symbol:
                if stock["total_shares"] < shares:
                    ...
                    quote = lookup(symbol)
                    if quote is None:
                        return apology("Unidentified symbol")

                    price = quote["price"]
                    total_sale = float(shares * price)
                    ...
                    flash(f"{shares} shares of {symbol} sold for ${usd(total_sale)}!")
                    return redirect("/")
